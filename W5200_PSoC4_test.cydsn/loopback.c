	...
	case SOCK_ESTABLISHED:					/* if connection is established */
		if(ch_status[s]==1)
		{
			// printf("\r\n%d : Connected",s);
                        ...
                        // printf("\r\n Peer Port : %d", ( (uint16_t)(IINCHIP_READ(Sn_DPORT0(s)+0)<<8) +(uint16_t)IINCHIP_READ(Sn_DPORT0(s)+1)));
			ch_status[s] = 2;
		}
...
void loopback_udp(SOCKET s, uint16_t port)
{
	...
	case SOCK_UDP:
		if ((RSR_len = getSn_RX_RSR(s)) > 0) 			/* check Rx data */
		{
			if (RSR_len > TX_RX_MAX_BUF_SIZE) RSR_len = TX_RX_MAX_BUF_SIZE;	/* if Rx data size is lager than TX_RX_MAX_BUF_SIZE */
																												/* the data size to read is MAX_BUF_SIZE. */
			received_len = recvfrom(s, data_buf, RSR_len, (uint8_t*)&destip, &destport);			/* read the received data */
			if(sendto(s, data_buf, received_len,(uint8_t*)&destip, destport) == 0)	/* send the received data */
			{
